...
export const updateProduct = asyncHandler(async (req, res) => {
    ...
    if (req.files && req.files['image']) {
        await safeUnlink(product.image); // Delete old image
        product.image = await processImage(req.files['image'][0].buffer, 'product');
    }
    if (req.files && req.files['productImages']) {
        // For simplicity, this replaces all gallery images. 
        // A more complex implementation could handle individual additions/deletions.
...
export const deleteProduct = asyncHandler(async (req, res) => {
    const product = await Product.findById(req.params.id);
    if (product) {
        // ✅ Use the safeUnlink helper to delete all associated images
...
export const getProducts = asyncHandler(async (req, res) => {
    ...
    if (pageCategory) {
        filter.pageCategory = pageCategory;
    }
