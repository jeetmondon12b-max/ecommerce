...
const fetchModalProducts = async ({ pageParam = 1, queryKey }) => {
    const [, categorySlug] = queryKey;
    const { data } = await axios.get(`${API_URL}/api/products?category=${categorySlug}&page=${pageParam}&limit=12`);
    return data;
};
...
const ProductModal = ({ categorySlug, categoryName, isOpen, onClose }) => {
    const {
        ...
        queryKey: ['modal-products', categorySlug],
        queryFn: fetchModalProducts,
        ...
        enabled: isOpen, // শুধুমাত্র মডাল খোলা থাকলেই query চলবে
    });
    ...
    const lastProductElementRef = useInfiniteScroll(fetchNextPage, hasNextPage, isLoading);
        ...
        <div className="fixed inset-0 bg-black bg-opacity-60 z-50 flex justify-center items-center p-4 animate-fade-in" onClick={onClose}>
            <div className="bg-white rounded-2xl shadow-xl w-full max-w-4xl h-[90vh] flex flex-col" onClick={e => e.stopPropagation()}>
                ...
                <div className="flex-grow overflow-y-auto p-6">
                            ...
                            {data.pages.map((page, i) => (
                                <Fragment key={i}>
                                    {page.products.map((product, index) => {
                                        const isLastElement = page.products.length === index + 1;
                                        return (
                                            <div ref={isLastElement ? lastProductElementRef : null} key={product._id}>
                                                <ProductCard product={product} />
                                            </div>
                                        );
                                    })}
                    ...
                    {!isLoading && !hasNextPage && <p className="text-center text-gray-500 mt-8">You've seen it all!</p>}
                </div>
            </div>
        </div>
    );
