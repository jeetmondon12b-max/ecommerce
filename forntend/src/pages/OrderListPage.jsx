...
const OrderListPage = () => {
    ...
    useEffect(() => {
        const fetchOrders = async () => {
            try {
                const config = { headers: { Authorization: `Bearer ${userInfo.token}` } };
                const { data } = await axios.get(`${API_URL}/api/admin/orders`, config); // Corrected API endpoint
                setOrders(data);
            } catch (error) {
                console.error("Failed to fetch orders", error);
            } finally {
    ...
    const handleStatusChange = async (orderId, newStatus) => {
        try {
            const config = { headers: { Authorization: `Bearer ${userInfo.token}` } };
            await axios.put(`${API_URL}/api/admin/orders/${orderId}/status`, { status: newStatus }, config); // Corrected API endpoint
            setOrders(prevOrders => prevOrders.map(order => order._id === orderId ? { ...order, orderStatus: newStatus } : order));
        } catch (error) {
            alert("Status update failed!");
        }
